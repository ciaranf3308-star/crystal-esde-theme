#!/usr/bin/env python3
"""Crystal v17.1.0 system-view PIL mock + check suite.

Third poster batch (gbc, psp). Renders the REAL system-view carousel geometry parsed
from theme-src/crystal/views.xml at 1280x960 with the real card PNGs, then
runs assertion checks:

  1. carousel XML config unchanged from v14 (massive dimming kept:
     unfocusedItemDimming 0.70, saturation 0.50, opacity 0.92)
  2. selected item 2x, full-color, dominant at carousel center
  3. non-selected items dimmed to near dark navy (lum < 55, B>R)
  4. gbc/psp cards match independent re-resize of the user-supplied PNGs
     and differ from the v17.0.0 marquee cards; other 15 cards
     byte-identical to HEAD (v17.0.0)
  5. the 4 poster-less systems (nes, steam, windows, xbox360) keep their
     marquee cards (byte-identical to HEAD, non-blank, no debug pixels)
  6. retired poster_on overlays gone: PNGs deleted, no live
     <image name="selectedPoster"> elements anywhere
  7. only the system-view changelog comment in views.xml changed;
     v17.0.0 gamelist section byte-identical to HEAD
  8. no debug circles / placeholder graphics in any card
...
# v14 originals kept verbatim
KEPT_POSTERS = ["gba", "gb", "nds", "snes", "wii", "wiiu"]
# v16 new
NEW_POSTERS = ["genesis", "ps2", "dreamcast", "psx", "megadrive", "xbox"]
# v16 replacements (old-style v14 cards retired for matching art)
REPLACED_POSTERS = ["n64", "gc", "n3ds"]
# v17.1 new
NEW17_POSTERS = ["gbc", "psp"]
POSTER_SYSTEMS = KEPT_POSTERS + NEW_POSTERS + REPLACED_POSTERS + NEW17_POSTERS
...
def main():
    ...
    # ---- check 1: XML config unchanged from v14 (massive dimming kept) ----
    ...
    check("xml unfocusedItemDimming 0.70 (massive, kept)",
          cfg["dimming"] == 0.70, f"got {cfg['dimming']}")
    check("xml unfocusedItemSaturation 0.50 (kept)",
          cfg["saturation"] == 0.50, f"got {cfg['saturation']}")
    check("xml unfocusedItemOpacity 0.92 (kept)", cfg["opacity"] == 0.92)
    views_head = subprocess.run(["git", "-C", REPO, "show", "HEAD:theme-src/crystal/views.xml"],
                                capture_output=True).stdout
    # v17.1 adds only a system-view changelog comment to views.xml.
    d = subprocess.run(["git", "-C", REPO, "diff", "HEAD", "--", "theme-src/crystal/views.xml"],
                       capture_output=True, text=True).stdout
    changed_lines = [l for l in d.splitlines() if l.startswith(("+", "-")) and not l.startswith(("+++", "---"))]
    print("views.xml changed lines:", len(changed_lines))
    check("views.xml diff is only the v17.1 changelog comment",
          len(changed_lines) == 9 and
          not any(l.lstrip("+").lstrip("-").lstrip().startswith("<") for l in changed_lines),
          f"changed={len(changed_lines)}")
    ...
    canvas = Image.open(os.path.join(CRYSTAL, "backgrounds/psp.webp")).convert("RGB")
    canvas = canvas.resize((W, H)).convert("RGBA")
    dr = ImageDraw.Draw(canvas, "RGBA")
    fb = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"
    fr = "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"
    dr.text((0.030 * W, 0.195 * H), "SONY", font=ImageFont.truetype(fb, 17), fill=(10, 47, 160))
    dr.text((0.030 * W, 0.230 * H), "PSP", font=ImageFont.truetype(fb, 38), fill=(20, 30, 60))
    dr.text((0.030 * W, 0.404 * H), "Console power in your pocket. UMD adventures.",
            font=ImageFont.truetype(fr, 18), fill=(20, 30, 60))
    dr.text((0.030 * W, 0.530 * H), "847", font=ImageFont.truetype(fb, 52), fill=(20, 30, 60))
    dr.text((0.030 * W, 0.613 * H), "HANDHELD \u2022 2005",
    ...
    # ---- carousel strip: 7 items, genesis selected ----
    strip = ["gbc", "genesis", "xbox", "psp", "snes", "nes", "steam"]
    check("strip has 7 items, psp selected", len(strip) == 7 and strip[3] == "psp")
    check("strip mixes 5 poster + 2 marquee systems",
          sum(s in POSTER_SYSTEMS for s in strip) == 5 and
          sum(s in MARQUEE_SYSTEMS for s in strip) == 2, str(strip))
    ...
    out.save(os.path.join(REPO, "work/proofs/mock_v17_1_full.png"))
    print("wrote work/proofs/mock_v17_1_full.png")

    # ---- check 2: selected composition ----
    left, top, wpx, hpx = drawn["psp"]
    ...
    check("no card pixels above scissor top (no clipping)",
          all(v[1] >= scissor_top - 1 for v in drawn.values()))
    ...
    sel_lum, *_ = region_lum(drawn["psp"])
    art = Image.open(os.path.join(CRYSTAL, "cards/psp.png")).convert("RGB")
    ...
    neigh_lums = [region_lum(drawn[s])[0] for s in strip if s != "psp"]
    ...
    # ---- check 5: poster->system mapping for all 17 ----
    MEDIA = os.path.expanduser("~/workspace/user/media_library/image")
    # user-supplied PNGs (identity visually confirmed before baking)
    NEW17_SRC = {
        "gbc": "f8/f8109cdf3ef01168f1c6b18e76474674af132506ad09bd3dd9e70183c1164659.png",
        "psp": "9c/9cfa0ae2d39a6718a84bd5f3e1d4c2e15617767240b5cea6c18823df978ee974.png",
    }
    ...
    for s, rel in NEW17_SRC.items():
        src = Image.open(os.path.join(MEDIA, rel)).convert("RGB")
        check(f"mapping {s}: source is exactly 3:4 RGB", src.size == (1086, 1448) and src.mode == "RGB",
              f"{src.size} {src.mode}")
        ...
        old = head_blob(f"theme-src/crystal/cards/{s}.png")
        check(f"mapping {s}: replaced the v17.0.0 marquee card",
              len(old) > 0 and old != open(os.path.join(CRYSTAL, f"cards/{s}.png"), "rb").read())

    for s in [x for x in POSTER_SYSTEMS if x not in NEW17_POSTERS]:
        old = head_blob(f"theme-src/crystal/cards/{s}.png")
        check(f"mapping {s}: byte-identical to v17.0.0",
              old == open(os.path.join(CRYSTAL, f"cards/{s}.png"), "rb").read())

    check("final card count: 17 poster cards",
          len(POSTER_SYSTEMS) == 17, f"got {len(POSTER_SYSTEMS)}")

    # ---- check 6: 4 marquee systems fall back cleanly ----
    check("remaining missing-card list is exactly the 4",
          sorted(MARQUEE_SYSTEMS) == ["nes", "steam", "windows", "xbox360"],
          str(sorted(MARQUEE_SYSTEMS)))
    for s in MARQUEE_SYSTEMS:
        ...
        head = head_blob(f"theme-src/crystal/cards/{s}.png")
        check(f"marquee {s}: byte-identical to v17.0.0", head == open(p, "rb").read())
    ...
    # ---- check 7: retired overlays gone, no live selectedPoster ----
    for s in ["dreamcast", "genesis", "megadrive", "ps2", "psx"]:
        p = os.path.join(CRYSTAL, f"art/poster_on/{s}.png")
        check(f"retired art/poster_on/{s}.png absent", not os.path.exists(p))
    live = []
    for s in ROSTER:
        t = open(os.path.join(CRYSTAL, f"{s}/theme.xml")).read()
        # v14 retirement comments in n64/gc theme.xml are allowed; a live
        # element is not.
        if re.search(r'<image name="selectedPoster">', t):
            live.append(s)
    check("no live selectedPoster elements anywhere", live == [], str(live))
    views_text = open(VIEWS).read()
    check("views.xml has no poster_on references", "poster_on" not in views_text)

    # ---- check 8: no debug/placeholder graphics in the 17 cards ----
    ...
    # ---- check 9: v15 gamelist work untouched ----
    ...
    check("gamelist XML section byte-identical to v17.0.0",
