#!/usr/bin/env python3
"""Crystal v17.7.0 asset generation: PRECISION / PRODUCT-DESIGN PASS.
...
- NO art-direction redesign. NO fallback artwork changes. NO new UI
  ...
  byte-identical to v17.6.0.
- Macro geometry locked. The only geometric moves are precision
  alignment fixes inside the locked footprints:
    * screenshot element x 42 -> 48 (aligns with the text column);
      the baked screenshot hairline / registration corner / kicker
      shift +6px in the panel to match.
...
def gen_panel():
    ...
    for text, xy in [("RELEASED", (42, 446)), ("RATING", (290, 446)),
                     ...
                     ("DEVELOPER", (234, 598)), ("SCREENSHOT", (42, 658))]:
        k = tracked_text_img(text, 9, SLATE, 4, alpha=130)
        body.alpha_composite(k, xy)

    # ---- SCREENSHOT: media, not a form field ----
    # image element local box: (42,690)-(438,900) - aligned with the
    # text column. No border box: one clean bottom-edge hairline,
    # subtle depth, ONE tiny yellow registration-mark corner crossing
    # the screenshot's top-right corner.
    fsh = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    ImageDraw.Draw(fsh).rectangle([44, 694, 440, 902], fill=(6, 14, 44, 30))
    body = Image.alpha_composite(body, fsh.filter(ImageFilter.GaussianBlur(3)))
    d = ImageDraw.Draw(body, "RGBA")
    d.line([(42, 900), (438, 900)], fill=ROYAL + (110,), width=1)
    d.line([(430, 682), (430, 696)], fill=YELLOW + (220,), width=2)
    d.line([(430, 682), (444, 682)], fill=YELLOW + (220,), width=2)
...
# ------------------------------------------------- the console spine ---
# 60x912: deep solid royal foundation, dotgrid FURTHER reduced
# (40px/alpha 3 - the one subtle texture area), ONE thin 1px continuous
# yellow structural line, supplied logos at modestly increased presence
# (width cap 36->40, length cap 560->620) centred OPTICALLY (6px above
...
def gen_rail(system, name):
    ...
    dotgrid(img, (6, 30, 54, 882), alpha=3, spacing=40)
...
def gen_hero_wash():
    ...
    for y in range(S):
        for x in range(S):
            ...
            if r < 0.45:
                t = 1 - r / 0.45
                a = int(20 * t * t)          # white lift, peak alpha 20
                px[x, y] = (235, 242, 255, a)
            elif r > 0.62:
                t = (r - 0.62) / 0.38
                a = int(26 * t * t)          # navy depth, peak alpha 26
                px[x, y] = (10, 24, 80, a)
    # soften the whole wash so no banding edge is visible
...
if __name__ == "__main__":
    ...
    print("rails:", kinds)
    with open(os.path.expanduser("~/workspace/crystal-esde-theme/work/rail_params_v17_7.json"), "w") as f:
        json.dump(RAIL_PARAMS, f, indent=1)
    print("rail params -> work/rail_params_v17_7.json")
