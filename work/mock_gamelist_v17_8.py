#!/usr/bin/env python3
"""Crystal v17.8.0 gamelist mock + verification (PIL renders, never ES-DE).

ROBUSTNESS PASS mock. The VM carries NO scraped artwork, so hero /
...
def render(v):
    ...
    f_title = ImageFont.truetype(BOLD, int(round(fs(0.0295))))
    title = v["title"].upper()
...
def check_rail_xml():
    ...
    f_rail = ImageFont.truetype(REG, int(round(fs(0.0145))))
    worst = {"libMetaGenre": "ACTION-ADVENTURE", "libMetaYear": "1998",
...
def check_screenshot_xml():
    ...
    check("screenshot rebalanced: x=48, y=724, h=170 (was 700/210)",
...
def check_hygiene():
    ...
    # v17.8 intentionally regenerates: panel, rails, hero plane, hero wash
    for f in ("lib_panel_main.png", "lib_plane_hero.png", "lib_hero_wash.png",
        ...
        if hashlib.sha256(h1).hexdigest() == hashlib.sha256(h2).hexdigest():
            ...
            return
    check("v17.8 assets regenerated (panel/rails/plane/wash)", True)
...
def check_proof(name, v, img, info):
    ...
    dev_lines = wrap(v["dev"].upper(), f_micro, dev_box_w)
...
def check_clearance_worst_case():
    ...
    d.text((450, 90 + (mcy - tcy)),
           "ACTION-ADVENTURE \u2022 1998 \u2022 NINTENDO ENTERTAINMENT ANALYSIS & DEVELOPMENT \u2022 1-8 PLAYERS",
...
if __name__ == "__main__":
    ...
    for name, v in VARIANTS.items():
        img, info = render(v)
        p = os.path.join(PROOFS, f"mock_v17_8_{name}.png")
        img.save(p)
        print("saved", p)
