#!/usr/bin/env python3
"""Crystal v16.0.0 system-view PIL mock + check suite.

Second poster batch. Renders the REAL system-view carousel geometry parsed
...
runs assertion checks:
  ...
  4. poster->system mapping for all 15 (cards match independent re-resize
     of the user-supplied JPGs; replacements differ from v14/v15 art;
     untouched v14 cards byte-identical to HEAD)
  5. the 6 poster-less systems keep their marquee cards (byte-identical
     to HEAD, non-blank, no debug pixels)
  6. retired poster_on overlays gone: PNGs deleted, no live
     <image name="selectedPoster"> elements anywhere
  7. views.xml byte-identical to HEAD; v15 gamelist work untouched
...
POSTER_SYSTEMS = KEPT_POSTERS + NEW_POSTERS + REPLACED_POSTERS
...
def main():
    ...
    check("views.xml byte-identical to v15 (raw)",
          views_head == open(VIEWS, "rb").read())
    ...
    canvas = Image.open(os.path.join(CRYSTAL, "backgrounds/genesis.webp")).convert("RGB")
    canvas = canvas.resize((W, H)).convert("RGBA")
    dr = ImageDraw.Draw(canvas, "RGBA")
    fb = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"
    fr = "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"
    dr.text((0.030 * W, 0.195 * H), "SEGA", font=ImageFont.truetype(fb, 17), fill=(10, 47, 160))
    dr.text((0.030 * W, 0.230 * H), "Genesis", font=ImageFont.truetype(fb, 38), fill=(20, 30, 60))
    dr.text((0.030 * W, 0.404 * H), "Blast-processing attitude. Sonic's 16-bit battleground.",
            font=ImageFont.truetype(fr, 18), fill=(20, 30, 60))
    dr.text((0.030 * W, 0.530 * H), "847", font=ImageFont.truetype(fb, 52), fill=(20, 30, 60))
    dr.text((0.030 * W, 0.613 * H), "16-BIT \u2022 1989",
    ...
    strip = ["nes", "xbox", "ps2", "genesis", "snes", "gbc", "psp"]
    check("strip has 7 items, genesis selected", len(strip) == 7 and strip[3] == "genesis")
    ...
    out.save(os.path.join(REPO, "work/proofs/mock_v16_full.png"))
    print("wrote work/proofs/mock_v16_full.png")

    # ---- check 2: selected composition ----
    left, top, wpx, hpx = drawn["genesis"]
    ...
    sel_lum, *_ = region_lum(drawn["genesis"])
    art = Image.open(os.path.join(CRYSTAL, "cards/genesis.png")).convert("RGB")
    ...
    neigh_lums = [region_lum(drawn[s])[0] for s in strip if s != "genesis"]
    ...
    # ---- check 5: poster->system mapping for all 15 ----
    MEDIA = os.path.expanduser("~/workspace/user/media_library/image")
    sys.path.insert(0, os.path.join(REPO, "work"))
    from gen_v14_assets import POSTERS as V14_POSTERS
    from gen_v16_assets import POSTERS as V16_POSTERS

    def head_blob(path):
        return subprocess.run(["git", "-C", REPO, "show", f"HEAD:{path}"],
                              capture_output=True).stdout

    for s, rel in V16_POSTERS.items():
        ...
        check(f"mapping {s}: replaced prior card",
              len(old) > 0 and old != open(os.path.join(CRYSTAL, f"cards/{s}.png"), "rb").read())

    for s in KEPT_POSTERS:
        src = Image.open(os.path.join(MEDIA, V14_POSTERS[s])).convert("RGB")
        expect = np.asarray(src.resize((240, 320), Image.Resampling.LANCZOS)).astype(np.float32)
        got = np.asarray(Image.open(os.path.join(CRYSTAL, f"cards/{s}.png")).convert("RGB")).astype(np.float32)
        diff = np.abs(expect - got).mean()
        check(f"mapping {s}: kept card still matches supplied poster", diff < 3.0,
              f"meandiff={diff:.2f}")
        old = head_blob(f"theme-src/crystal/cards/{s}.png")
        check(f"mapping {s}: byte-identical to v15",
              old == open(os.path.join(CRYSTAL, f"cards/{s}.png"), "rb").read())

    check("final card count: 15 poster cards",
          len(POSTER_SYSTEMS) == 15, f"got {len(POSTER_SYSTEMS)}")

    # ---- check 6: 6 marquee systems fall back cleanly ----
    check("remaining missing-card list is exactly the 6",
          sorted(MARQUEE_SYSTEMS) == ["gbc", "nes", "psp", "steam", "windows", "xbox360"],
    ...
    for s in MARQUEE_SYSTEMS:
        ...
        check(f"marquee {s}: byte-identical to v15", head == open(p, "rb").read())
    ...
    # ---- check 8: no debug/placeholder graphics in the 15 cards ----
    ...
    check("gamelist XML section byte-identical to v15",
